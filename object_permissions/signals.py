...
view_add_user = django.dispatch.Signal(providing_args=["user", "group"])

# sent when a user has been remove from a group
view_remove_user = django.dispatch.Signal(providing_args=["user", "group"])

# send when a user's permissions have been edited
view_edit_user = django.dispatch.Signal(providing_args=["user", "obj"])